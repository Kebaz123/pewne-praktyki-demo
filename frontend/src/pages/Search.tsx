...
const Search = () => {
  ...
  // Usunięte: offerData, cityOptions, sortedOccupations, selectedOccupations
  // Zostawiamy tylko stany potrzebne do UI
  const [selectedType, setSelectedType] = useState<string>(t.greenBox.all);
  const [selectedCity, setSelectedCity] = useState<string>(t.greenBox.all);
  const [searchText, setSearchText] = useState<string>('');
  ...
  // Tylko jedna opcja miasta na razie
  const cityOptions = [t.greenBox.all];
  ...
  return (
    <div id="searchPage">
      ...
      <div id="optionPanel" className={filtersOpen ? 'panelOpen' : ''}>
        ...
        {/* Usunięta sekcja z sortedOccupations */}
      </div>
      ...
      <div id="mainPanel">
        ...
        <div id="offerList">
          <p id="noData">{t.noData}</p>
          {/* Tymczasowo pokazujemy informację o braku danych */}
          <div style={{ textAlign: 'center', padding: '2rem' }}>
            <p>Brak danych do wyświetlenia - backend nie jest podłączony</p>
          </div>
        </div>
      </div>
    </div>
